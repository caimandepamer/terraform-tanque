...
data "template_file" "network_config" {
  ...
  vars = {
       IPv4Address = "192.168.0.${250 + count.index}"
  }
}
...
resource "null_resource" "ansible-ping" {
 ...
 provisioner "local-exec" {
    command = "ansible-playbook  -i inventory-dev-simple wait_until_servers_are_up.yml"
  }
}
#resource "null_resource" "ansible-install-k8s" {
# depends_on = [null_resource.ansible-ping]
# provisioner "local-exec" {
#    command = "ansible-playbook  -i inventory-dev-simple todos.yml"
#  }
#}
...
# provisioner "local-exec" {
#    command = "ansible-playbook  -i inventory-dev-simple solo-masters.yml"
#  }
#}
...
# provisioner "local-exec" {
#    command = "ansible-playbook  -i inventory-dev-simple workers.yml"
# }
#}
